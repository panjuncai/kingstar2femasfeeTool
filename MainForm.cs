...
namespace kingstar2femasfee
{
    public partial class MainForm : Form
    {
        ...
        private bool ProcessKingstarSpecialTradeFeeData(string kingstarDir)
        {
            ...
            var (successProduct, dataListProduct) = ExcelHelper.ReadKingstarSpecialTradeFeeExcelProduct(kingstarDir, LogInfo);
            if (!successProduct)
            {
                LogInfo("没有金士达品种手续费率文件");
            }
            ...
            // 合并数据
            dataList = dataList.Concat(dataListOptions).Concat(dataListProduct).ToList();
        ...
        private void RefreshKingstarSpecialFeeDataGridView()
        {
            try
            {
                ...
                var displayData = dataList.Select(data =>
                {
                    ...
                    {
                        LogInfo($"处理单条数据时发生错误: {ex.Message}");
                        LogInfo($"错误数据详情: InvestorId={data.InvestorId}, ExchCode={data.ExchCode}, ProductType={data.ProductType}");
                        throw; // 重新抛出异常，以便上层捕获
                    }
                }
                    ).ToList();
        ...
        private List<Dictionary<string, string>> PrepareExportData()
        {
            ...
            foreach (DataGridViewRow row in dataGridView_femas_special_fee_export.Rows)
            {
                ...
                rowData["跟随交易所倍数"] = GetCellValue(row, "新是否跟随")=="跟随" ? "1" : "0";
