-- Create table
create table T_SPECIAL_TRADE_FEE
(
  id primary key,
  ...
  follow_type                  VARCHAR2(1),
  multiple_ratio               NUMBER(17,8),
...
comment on column T_SPECIAL_TRADE_FEE.product_id
  is '产品代码';
comment on column T_SPECIAL_TRADE_FEE.option_series_id
  is '期权系列';
comment on column T_SPECIAL_TRADE_FEE.instrument_id
  is '合约代码';
comment on column T_SPECIAL_TRADE_FEE.hedge_flag
  is '投保标识';
comment on column T_SPECIAL_TRADE_FEE.buy_sell
  is '买卖标识';
...
comment on column T_SPECIAL_TRADE_FEE.exec_clear_fee_amt
  is '行权手续费额（按手数）';
comment on column T_SPECIAL_TRADE_FEE.follow_type
  is '是否跟随交易所';
comment on column T_SPECIAL_TRADE_FEE.multiple_ratio
  is '跟随交易所倍数';
...
alter table T_SPECIAL_TRADE_FEE
  add unique index idx_SPECIAL_TRADE_FEE (INVESTOR_ID, EXCH_CODE, PRODUCT_TYPE, PRODUCT_ID, OPTION_SERIES_ID, INSTRUMENT_ID, HEDGE_FLAG, BUY_SELL);
